import { millisecondsToHoursMinutes } from "./utilities.js";
...
const hourlyChartContainer = document.getElementsByClassName('hourly-chart-container')[0];
const pieChartContainer = document.getElementsByClassName('pie-chart-container')[0];
...
async function fetchUserSessionsByDomains(startTime, endTime){
    const userSessionsList = await fetch(`${API_URL}/user_sessions?start_time=${startTime}&end_time=${endTime}`, options)
      .then((res)=>{
...
async function fetchUserSessionsForDomain(domainName, startTime, endTime){
  const userSessionsList = await fetch(`${API_URL}/user_sessions/${domainName}?start_time=${startTime}&end_time=${endTime}`, options)
  .then((res)=>{
    if(!res.ok) throw new Error("failed to fetch the resources");
    return res.json();
  })
...
async function fetchDomainSelectionList(){
  const domainSelectionList = await fetch(`${API_URL}/domain_list`)
  .then(res=>{
    if(!res.ok) throw new Error("failed to fetch the resources");
    return res.json();
})
...
async function fetchHourlyUserSessionsWithinDay(startTimeOfDay){
  const res = await fetch(`${API_URL}/user_sessions_hourly?start_time_of_day=${startTimeOfDay}`, options);
  if (!res.ok) throw new Error("failed to fetch the resources");
  const data = await res.json();
  return data;
}
async function fetchHourlyUserSessionsWithinDayForDomain(domainName, startTimeOfDay){
  const hourlyUserSessionsList = await fetch(`${API_URL}/user_sessions_hourly/${domainName}?start_time_of_day=${startTimeOfDay}`, options)
      .then((res)=>{
        if(!res.ok) throw new Error("failed to fetch the resources");
        return res.json();
      })
...
function createHourlyBarChart(startTimeOfDay, domainName) {
  ...
  if(domainName)
    hourlyUserSessionList =  fetchHourlyUserSessionsWithinDayForDomain(domainName,startTimeOfDay);
  ...
  hourlyUserSessionList.then(data=>{
    existingBarChart = new Chart(hourlyChart, {
      ...
      data: {
        labels: hoursArray,
        datasets: [{
          label: domainName?`hourly time consumption for ${domainName}`: `hourly time consumption`,
          data : data,
          borderWidth: 1,
...
function createDomainConsumptionList(startTime, endTime, domainName){
  ...
  if(domainName)
  userSessionsList = fetchUserSessionsForDomain(domainName,startTime, endTime);
  ...
  userSessionsList.then((response=>{
    ...
    response.forEach(item => {
      ...
      li.addEventListener('click', ()=>{
        ...
        createDomainConsumptionList(currentDate, currentDate+24*3600*1000-1, currentDomain);
      })
      ...
      totalTimeSpentHeader.textContent = millisecondsToHoursMinutes(item.totalTimeSpent);
...
lastNTimeBtnEle.addEventListener('click',()=>{
  datePickerEle.value = getCurrentDateString();
  const newLastNTime= timeToMilliseconds(parseInt(lastNHourEle.value)||0,parseInt(lastNMinuteELE.value));
