import { millisecondsToMinutes } from "./utilities.js";
...
function setUserIdCookie(userId) {
  const cookieDetails = {
    url: 'http://192.168.48.1:8080/', // Replace with the domain you want to set the cookie for
    name: 'userId',
    value: userId,
    path:"/",
...
function captureVisibleTab(){
  chrome.tabs.captureVisibleTab(null, {quality:4},function(screenshotDataUrl) {
...
function startTracking(url, changeInfo){
 ...
 //set timelimit 
chrome.storage.local.set({"www.google.com":10000});
chrome.storage.local.get(["www.google.com"],res=>{
  console.log("Alarm set for www.google.com");
})

chrome.storage.local.set({"github.com":10000});
chrome.storage.local.get(["github.com"],res=>{
  console.log("Alarm set for github.com");
})

function stopTracking(){
  chrome.storage.local.get(["currentHostname","snapshots","startTime","uniqueUserId"],function({currentHostname,snapshots,startTime,uniqueUserId}){
...
chrome.tabs.onUpdated.addListener(
  function(tabId,changeInfo, tab){
      if(changeInfo.status ==="complete")
        startTracking(tab.url, changeInfo);
    }
)
...
function sendToApi(data){
  ...
  fetch(`${apiURL}/user_sessions`, requestOptions)
    .then((response) => {
      if (!response.ok) {
